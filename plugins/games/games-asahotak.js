...
module.exports = {
  ...
  description: 'Menampilkan soal asah otak (pengetahuan umum atau logika) beserta jawabannya.',
  params: [],
  handler: async (requestParameter, getInputFunction, responseObject) => {
    try {
      const upstreamUrl = 'https://api.siputzx.my.id/api/games/asahotak';
      ...
      if (responseData.status !== true || !responseData.data) return { ok: false, status: 404, message: 'Data asah otak tidak ditemukan.' };
