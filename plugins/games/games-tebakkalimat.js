...
module.exports = {
  id:          'games-tebakkalimat',
  name: 'Tebak Kalimat',
  category:    'Games',
  method: 'GET',
  path:        '/api/games/tebakkalimat',
  description: 'Menampilkan soal melengkapi peribahasa atau kalimat terkenal beserta jawabannya.',
  params: [],
  handler: async (requestParameter, getInputFunction, responseObject) => {
    try {
      const upstreamUrl = 'https://api.siputzx.my.id/api/games/tebakkalimat';
      ...
      if (responseData.status !== true || !responseData.data) return { ok: false, status: 404, message: 'Data tebak kalimat tidak ditemukan.' };
      const { index, soal, jawaban } = responseData.data;
      return { ok: true, status: 200, index, question: soal, answer: jawaban, timestamp: responseData.timestamp || new Date().toISOString() };
