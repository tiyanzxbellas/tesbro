...
module.exports = {
  id:          'games-tebakkartun',
  name: 'Tebak Kartun',
  category:    'Games',
  method: 'GET',
  path:        '/api/games/tebakkartun',
  description: 'Menampilkan gambar karakter kartun, pengguna harus menebak nama karakternya.',
  params: [],
  handler: async (requestParameter, getInputFunction, responseObject) => {
    try {
      const upstreamUrl = 'https://api.siputzx.my.id/api/games/tebakkartun';
      ...
      if (responseData.status !== true || !responseData.data) return { ok: false, status: 404, message: 'Data tebak kartun tidak ditemukan.' };
      const { name, img } = responseData.data;
      return { ok: true, status: 200, character_name: name, image_url: img, timestamp: responseData.timestamp || new Date().toISOString() };
