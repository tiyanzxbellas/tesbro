...
module.exports = {
  ...
  description: 'Menampilkan potongan lirik lagu yang hilang satu kata, pengguna harus menebak kata tersebut.',
  params: [],
  handler: async (requestParameter, getInputFunction, responseObject) => {
    try {
      const upstreamUrl = 'https://api.siputzx.my.id/api/games/tebaklirik';
      ...
      if (responseData.status !== true || !responseData.data) return { ok: false, status: 404, message: 'Data tebak lirik tidak ditemukan.' };
      const { soal, jawaban } = responseData.data;
      return { ok: true, status: 200, lyric_blanks: soal, answer: jawaban, timestamp: responseData.timestamp || new Date().toISOString() };
